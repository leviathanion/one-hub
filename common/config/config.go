...
func defaultConfig() {
	...
	viper.SetDefault("realtime.websocket_read_limit", int64(16<<20))
	viper.SetDefault("realtime.websocket_ping_interval_ms", 25000)
	viper.SetDefault("realtime.websocket_write_timeout_ms", 10000)
	viper.SetDefault("responses_ws.connect_per_credential_per_minute", 30)
	viper.SetDefault("responses_ws.active_lease_redis_fail_open", true)
	viper.SetDefault("responses_ws.first_frame_timeout_ms", 5000)
