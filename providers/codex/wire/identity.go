...
func ResolveIdentity(in IdentityInput) (Identity, []Decision, error) {
	...
	identity, decisions, err := ResolveClientIdentity(in.Headers, in.Policy)
	if err != nil {
		return Identity{}, nil, err
	}

	identity.SessionID = headerMetadataOrAutoGenerated(in.Headers, in.Metadata, "session-id", "session_id", validID, in.Policy.AutoGenerate.SessionID, &decisions, identity.Sources)
	identity.ThreadID = headerMetadataOrAutoGenerated(in.Headers, in.Metadata, "thread-id", "thread_id", validID, in.Policy.AutoGenerate.ThreadID, &decisions, identity.Sources)
	identity.ClientRequestID = headerOrAutoClientRequestID(in.Headers, identity.ThreadID, in.Policy.AutoGenerate.ClientRequestID, &decisions, identity.Sources)
...
func headerOrAutoClientRequestID(headers HeaderSnapshot, fallbackThreadID string, autoGenerate bool, decisions *[]Decision, sources map[string]Source) string {
	const name = "x-client-request-id"
	header := headers.Singleton(name, validID)
	...
	value := strings.TrimSpace(fallbackThreadID)
	reason := "thread-id"
	if value == "" {
		value = uuid.NewString()
		reason = "identity-missing"
	}
	*decisions = append(*decisions, valueDecision(name, "fallback", SourceGenerated, reason, value))
	sources[name] = SourceGenerated
	return value
}

func headerMetadataOrAutoGenerated(headers HeaderSnapshot, metadata Metadata, headerName, metadataName string, valid func(string) bool, autoGenerate bool, decisions *[]Decision, sources map[string]Source) string {
...
func resolveInstallationID(in IdentityInput, sessionID string, decisions *[]Decision, sources map[string]Source) (string, error) {
	...
	if in.Policy.AutoGenerate.InstallationID {
